...
TaxTrace activates A/B and C as separate logical requests so their distinct source grains remain explicit. For FY2025/P12, File C uses a pinned, independently verified official all-agency USAspending archive generated from the exact 14 columns consumed by Federal Product V2. The archive reached terminal success, contains 39,777,645 logical rows across 42 CSV members, and is materialized locally as 42 Parquet objects. The logical File C request remains all-agency Federal Account × award at period 12; the pinned official archive is recorded separately as transport provenance. Periods without a verified completed-year product release fall back to the exact current-reporting-agency generation path, and every agency is still required. TaxTrace refuses incomplete federal coverage.
