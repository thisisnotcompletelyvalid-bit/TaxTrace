...
def test_census_registry_and_finance_ingest(db_session, tmp_path):
    ...
    police = db_session.scalar(
        ...
        .where(GovernmentFinanceFact.jurisdiction_id == city.id, FinanceClassification.code == "E62")
    )
    assert police is not None
    assert police.amount == Decimal("1234000.00")
    classification = db_session.scalar(select(FinanceClassification).where(FinanceClassification.code == "E62"))
